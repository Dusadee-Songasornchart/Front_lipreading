...
const Team = () => {
  ...
  const sendToken = () => {
    setLoading(true);
    const api =
      process.env.NEXT_PUBLIC_LINKTEST + "/web-admin/team_getAllTeamInfo";
  ...
  const editmember = (user_id, status) => {
    ...
    const api = process.env.NEXT_PUBLIC_LINKTEST + "/web-admin/team_editTeam";
    axios
      .post(api, {
        access_token: token,
        changed_team_id: user_id,
        status: status,
      })
      .then(function (response) {
        console.log(response);
  ...
  const handleSearch = () => {
    const data = defaultValues.teams;
    console.log(searchMember);
    if (searchMember !== "") {
      const filteredNames = data.filter(
        (name) =>
          name.team_name.toLowerCase().includes(searchMember.toLowerCase()) ||
          name.owner.toLowerCase().includes(searchMember.toLowerCase())
      );
      ...
      <div className="invisible w-0 h-0 lg:visible lg:w-auto lg:h-auto">
        ...
        <div className=" mt-[6.5rem] w-full">
          <div className="ml-[10rem] flex justify-between">
            <div className="text-blue-900 font-bold text-4xl mt-2">Team</div>
            <div>
              <label
          ...
          <div className="mt-5 flex justify-between py-7 bg-sky-50 rounded-md text-gray-900 font-bold text-xl">
            <div className="flex flex-rows ">
              <div className="ml-[10rem]">Teamid</div>
              <div className="ml-[4rem]">Teamname</div>
              <div className="ml-[9rem]">Owner</div>
            </div>
            <div className="flex flex-rows">
              <div className="md:px-10">member</div>
          ...
          <div>
            {showMemberSearch ? (
              <div>
                {showMemberSearch.length !== 0 ? (
                  <ul className="ml-[8rem]  space-y-4  mt-4">
                    {showMemberSearch.map((team, index) => (
                      <li key={index}>
                        <div className=" static flex items-center justify-between w-auto py-4 border-b-2 ">
                          <div className="flex flex-row items-center">
                            ...
                            <div className="absolute left-[35rem]">
                              {team.owner}
                            </div>
                          </div>
                          <div className="flex flex-row items-center">
                            <div className="mr-[8rem]">
                              {team.member.count_member}
                            </div>
                            ...
                            <div className="mr-5">
                              <select
                                ...
                                value={team.status}
                                onChange={(event) =>
                                  handleStatusChage(team.team_id, event)
                                }
                              >
              ...
              <ul className="ml-[8rem] space-y-4  mt-4">
                {defaultValues.teams.map((team, index) => (
                  <li key={index}>
                    <div className=" static flex items-center justify-between w-auto py-4 border-b-2 ">
                      <div className="flex flex-row items-center">
                        <div className="ml-3 md:ml-[4rem] text-xl">
